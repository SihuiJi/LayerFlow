...
def attention_forward_default(self, hidden_states, mask, **kw_args):
    ...
    mixed_raw_layer = self.query_key_value(hidden_states,scale=kw_args['scale'] if 'scale' in kw_args else 1)
    (mixed_query_layer,
        mixed_key_layer,
    ...
    # query_layer.shape torch.Size([2, 30, 16878, 64])
    context_layer = attention_fn(query_layer, key_layer, value_layer, mask, dropout_fn, **kw_args)
    # print("context_layer", context_layer.shape)
    # context_layer torch.Size([2, 30, 16878, 64]
    context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
    new_context_layer_shape = context_layer.size()[:-2] + (self.hidden_size_per_partition,)
    context_layer = context_layer.view(*new_context_layer_shape)
    output = self.dense(context_layer,scale=kw_args['scale'] if 'scale' in kw_args else 1)
