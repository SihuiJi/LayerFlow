...
from base_model import BaseModel, non_conflict, BaseMixin
from transformer_defaults import HOOKS_DEFAULT, attention_fn_default
...
class ImagePatchEmbeddingMixin_layer(BaseMixin):
    def __init__(
        self,
        in_channels,
        hidden_size,
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        if self.text_proj is not None:
            ...
            if text_emb.shape[0]!=emb.shape[0]:
                text_emb = rearrange(text_emb, "(b t) n d -> b (t n) d", b=emb.shape[0])
...
class Basic3DPositionEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        learnable_txt_embed=False,
        pose_hidden_size=1920
    ):
        super().__init__()
        self.height = height
        self.width = width
        self.text_length = text_length
        self.txt_num = text_length//226
        ...
        if learnable_pos_embed:
            self.pos_embedding = nn.Parameter(
                torch.zeros(1, int(text_length + self.num_patches), int(hidden_size)), requires_grad=True
            )
        else:
            self.pos_embedding = nn.Parameter(
                torch.zeros(1, int(text_length + self.num_patches), int(hidden_size)), requires_grad=False
            )
        if learnable_txt_embed:
            self.txt_embedder = nn.Sequential(
                                nn.Linear(1, int(hidden_size)),
                                nn.LayerNorm(int(hidden_size)))
            self.txt_embedder.requires_grad_(True)
        else:
            self.txt_embedder = None
    ...
    def position_embedding_forward(self, position_ids, **kwargs):
        ...
        if not self.txt_embedder == None:
            txt_embeddings = []
            for i in range(self.txt_num):
                txt_embeddings.append(self.txt_embedder((torch.ones(226,1)*i).to(device=self.pos_embedding.device, dtype=self.pos_embedding.dtype)))
            txt_embeddings = torch.cat(txt_embeddings, dim=0).unsqueeze(0)
            txt_embeddings = torch.cat([txt_embeddings[:, : self.text_length],torch.zeros(1, int(self.num_patches), int(self.hidden_size)).to(device=self.pos_embedding.device, dtype=self.pos_embedding.dtype)],dim=1)
        else:
            txt_embeddings = torch.zeros_like(self.pos_embedding[:, : self.text_length + kwargs["seq_length"]])
        return self.pos_embedding[:, : self.text_length + kwargs["seq_length"]]+txt_embeddings
...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        **kwargs,
        ...
        attention_output = layer.attention(attention_input, mask, **kwargs) #SelfAttention
...
class DiffusionTransformer(BaseModel):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if "cat_mode" in kwargs and kwargs["cat_mode"]==True:
            x = rearrange(x, 'b (f l) c h w -> b l (f c) h w', l=4)
        b, t, d, h, w = x.shape
        ...
        if "cond_input" in kwargs and kwargs["cond_input"] is not None:
            if kwargs["cond_input"].shape[0] != x.shape[0]:
                cond_input = kwargs["cond_input"].repeat(2, 1, 1, 1, 1)
            else:
                cond_input = kwargs["cond_input"]
            if 'task_idx' in kwargs and 1<=kwargs["task_idx"]<=3:
                task_idx = kwargs["task_idx"]
                if task_idx ==1:
                    x = torch.cat([cond_input, x], dim=1)
                elif task_idx ==2:
                    x = torch.cat([x[:, :4], cond_input, x[:, -4:]], dim=1)
                elif task_idx ==3:
        ...
        if context.shape[0]!= emb.shape[0]:
            kwargs["text_length"] = context.shape[0]//emb.shape[0]*context.shape[1]
        else:
        ...
        if "cat_mode" in kwargs and kwargs["cat_mode"]==True:
            output = rearrange(output, 'b l (f c) h w -> b (f l) c h w', c=16)
        if "cond_input" in kwargs and kwargs["cond_input"] is not None:
            if 'task_idx' in kwargs and 1<=kwargs["task_idx"]<=3:
                task_idx = kwargs["task_idx"]
                if task_idx ==1:
                    ...
                    output = torch.cat([output[:, :4],output[:, -4:]],dim=1)
                elif task_idx ==3:
                    output = output[:,:-4]
            else:
                output = output[:,:-2]
