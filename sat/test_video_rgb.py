...
from sat import mpu
from deepspeed_test import test_main

from sgm.util import get_obj_from_str, isheatmap

from diffusion_video_rgb import SATVideoDiffusionEngine
...
def save_video_as_grid_and_mp4(video_batch: torch.Tensor, save_path: str, T: int, fps: int = 5, args=None, key=None,iteration=0):
    ...
    for i, vid in enumerate(video_batch):
        ...
        now_save_path = os.path.join(save_path, f"{iteration:06d}.mp4")
...
def log_video(batch, model, args, only_log_video_latents=False,iteration=0):
    texts = batch["txt"]
    texts_new = []
    if isinstance(texts[0], list):
        for i in range(len(texts)):
            texts_new.append(texts[i][0])
    ...
    with torch.no_grad(), torch.cuda.amp.autocast(**gpu_autocast_kwargs):
        videos = model.log_video(batch, only_log_video_latents=only_log_video_latents,iteration=iteration)
        ...
        if only_log_video_latents:
            ...
            if only_log_video_latents:
                ...
                for k in videos:
                    ...
                    path = os.path.join(root, filename)
                    os.makedirs(os.path.split(path)[0], exist_ok=True)
                    save_video_as_grid_and_mp4(samples, path, num_frames // fps, fps, args, k, iteration=iteration)
...
def broad_cast_batch(batch):
    ...
    if mpu.get_model_parallel_rank() != 0:
        batch["mp4"] = torch.zeros(mp4_shape, device="cuda:4")
        batch["fps"] = torch.zeros(fps_shape, device="cuda:4", dtype=torch.long)
        batch["num_frames"] = torch.zeros(num_frames_shape, device="cuda:4", dtype=torch.long)
...
def forward_step_eval(data_iterator, model, args, timers, only_log_video_latents=False, data_class=None, iteration=0):
    if mpu.get_model_parallel_rank() == 0:
    ...
    log_video(batch_video, model, args, only_log_video_latents=only_log_video_latents,iteration=iteration)
    torch.distributed.barrier()
