...
# samples = self.sampler(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb, is_stage1 = self.denoiser.is_stage1)
...
# here!!!
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None, is_stage1=False,cond_image=None,task_idx=0,cat_mode=False):
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            if is_stage1:
                additional_model_inputs["scale"] = 1
            else:
                additional_model_inputs["scale"] = 0
            additional_model_inputs["idx"] = torch.cat([x.new_ones([x.shape[0]]) * timestep] * 2)
            if cond_image is not None:
                # print('cond_image', cond_image.shape)
                # cond_image torch.Size([1, 2, 16, 60, 90])
                additional_model_inputs["cond_input"] = torch.cat([cond_image] * 2)
                additional_model_inputs["task_idx"] = task_idx
            if cat_mode:
                additional_model_inputs["cat_mode"] = True
            denoised = denoiser(
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, is_stage1=False):
...
# here
    def sampler_step(
        ...
        denoised = self.denoise(
            x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb,is_stage1=is_stage1,cond_image=cond_image,task_idx=task_idx,cat_mode=cat_mode
        ).to(torch.float32)
        if idx == 1:
...
# here
    # samples = self.sampler(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb, is_stage1 = self.denoiser.is_stage1)
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None, is_stage1=False, is_seg=False, cond_image=None, task_idx=0, cat_mode=False):
        x, s_in, alpha_cumprod_sqrt, num_sigmas, cond, uc, timesteps = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
        # print('x',x.shape)
        # print(self.fixed_frames)
        # 4
        # torch.Size([1, 6, 16, 40, 60])
        if self.fixed_frames > 0 and is_seg:
            # prefix_frames = x[:, : self.fixed_frames]
            prefix_frames = x[:, self.fixed_frames:]
            # print('prefix_frames',prefix_frames.shape)
        ...
        for i in self.get_sigma_gen(num_sigmas):
            if self.fixed_frames > 0 and is_seg:
                if self.sdedit:
                    ...
                    # rd = torch.zeros_like(prefix_frames)
                    ...
                    x = torch.cat([x[:, :self.fixed_frames], noised_prefix_frames], dim=1)
                else:
                    # print('here')
                    # print('prefix',prefix_frames.sum())
                    x = torch.cat([x[:, :self.fixed_frames], prefix_frames], dim=1)
                    # if old_denoised!=None:
                    #     if i == 1:
                    #         prefix_old_frames = old_denoised[:, self.fixed_frames:]
                    #     else:
                    #         # print('here')
                    #         old_denoised = torch.cat([old_denoised[:, :self.fixed_frames], prefix_old_frames], dim=1)
            # print("x-------", x.shape)
            # 1 6 16 40 60
            # if old_denoised!=None:
                # print(f'old_denoised{i}',old_denoised.shape)
                # torch.Size([1, 6, 16, 60, 90])
            x, old_denoised = self.sampler_step(
                old_denoised,
        ...
        if self.fixed_frames > 0 and is_seg:
            # print('prefix_frames',prefix_frames.sum())
            x = torch.cat([x[:, :self.fixed_frames],prefix_frames], dim=1)
