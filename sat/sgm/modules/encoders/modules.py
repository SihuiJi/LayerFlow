...
class GeneralConditioner(nn.Module):
    ...
    def get_single_embedding(
        ...
        with embedding_context():
            if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                ...
                # embed()
                # print('batch[embedder.input_key]',batch[embedder.input_key])
                # ["",""]
                # None
    ...
    def forward(self, batch: Dict, force_zero_embeddings: Optional[List] = None) -> Dict:
        ...
        # print('batch',batch)
        # 
        # print('self.cor_p',self.cor_p)
        # []
        # print('self.cor_embs',self.cor_embs)
        # []
        # print('self.embedders',self.embedders)
        # FrozenT5Embedder
        ...
        # print('output', output['crossattn'].shape)
        # torch.Size([8, 226, 4096])
        # torch.Size([4, 226, 4096])
        return output
    ...
    def get_unconditional_conditioning(self, batch_c, batch_uc=None, force_uc_zero_embeddings=None):
        ...
        c = self(batch_c)
        # print('batch_uc', batch_uc)
        # None
        # print('force_uc_zero_embeddings',force_uc_zero_embeddings)
        # ['txt']
    ...
    def get_unconditional_conditioning_only(self, batch_c, batch_uc=None, force_uc_zero_embeddings=None):
        if force_uc_zero_embeddings is None:
            force_uc_zero_embeddings = []
        ucg_rates = list()
        for embedder in self.embedders:
...
class FrozenT5Embedder(AbstractEmbModel):
    ...
    def forward(self, text):
        ...
        if not isinstance(text[0], list):
            ...
            # tokens torch.Size([2, 226])
            with torch.autocast("cuda:4", enabled=False):
            ...
            bs = len(text[0]) #2
            frames = len(text) #4
            # print('frames:', frames, ' bs:', bs)
            # frames: 4  bs: 2
            for i in range(bs):
                text_new = [text[j][i] for j in range(frames)]
                # print('text_new:', text_new)
                batch_encoding_i = self.tokenizer(
                    text_new,
                    truncation=True,
                    max_length=self.max_length,
                    return_length=True,
                ...
                tokens_i = batch_encoding_i["input_ids"].to(self.device)
                with torch.autocast("cuda", enabled=False):
                    outputs_i = self.transformer(input_ids=tokens_i)
                z_i = outputs_i.last_hidden_state
                z.append(z_i)
            z = torch.cat(z, dim=0)
        
        # print('z', z.shape)
        # torch.Size([8, 226, 4096])
        # z torch.Size([4, 226, 4096])
        return z
